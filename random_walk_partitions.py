...
def equality_histogram(graph, step_count, num_runs):
    ...
    for i in range(num_runs):
        ...
        score = abs(len(partitioning[0]) - len(partitioning[1]))
...
The major issues for low walk counts in that they would return to the boundary too quickly and therefore 
there would be a very large partition 1 in most cases and small partition 2. Large values had the opposite 
problem in which the walk would create many small areas and the random sampling would often create very small 
partition 1s. Upon talking to Justin it appears like a good next step would be to look at making the random walk
respond to some sort of global potential we update on every iteration. Lorenzo suggested using a Brownian Bridge 
